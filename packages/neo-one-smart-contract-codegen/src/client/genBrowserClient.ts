...
import { browserLocalClient } from '@neo-one/local';
...
export const createClient = (getUserAccountProviders = getDefaultUserAccountProviders) => {
  ...
  providers.push(new NEOONEDataProvider({ network: '${localDevNetworkName}', rpcURL: new JSONRPCLocalProvider() }));
...
export const createDeveloperClients = () => ({${networks
      ...
      .join('')}
  '${localDevNetworkName}': new DeveloperClient(new NEOONEDataProvider({ network: '${localDevNetworkName}', rpcURL: new JSONRPCLocalProvider() })),
});
